...
void Particle::Draw() {
	...
	if(!instancingTransfomationMatrixBuffer_) {
	...
	commandList->SetGraphicsRootConstantBufferView(1, instancingTransfomationMatrixBuffer_->GetGPUVirtualAddress());
	//並行光源の設定
...
void Particle::CreateTransformationMatrixBuffer() {
	//wvp用のリソースを作る
	instancingTransfomationMatrixBuffer_ = particleSetup_->GetDXManager()->CreateBufferResource(sizeof(TransformationMatrix) * kNumInstances);
	//書き込み用変数
	TransformationMatrix transformationMatrix = {};
	//書き込むためのアドレスを取得
	instancingTransfomationMatrixBuffer_->Map(0, nullptr, reinterpret_cast<void**>( &transformationMatrixData_ ));
	//書き込み
